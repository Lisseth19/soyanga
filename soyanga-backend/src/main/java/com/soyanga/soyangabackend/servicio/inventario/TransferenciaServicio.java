...
public class TransferenciaServicio {
        ...
        private void audit(String accion, TransferenciaEntreAlmacenes t, String detalle) {
                auditoriaServicio.registrar(
                                Auditoria.builder()
                                                .fechaEvento(LocalDateTime.now())
                                                .idUsuario(currentUserId())
                                                .moduloAfectado("inventario.transferencias")
                                                .accion(accion) // p.ej.: crear_pendiente, confirmar_salida,
                                                                // confirmar_ingreso, crear_completa,
                                                                // anular
                                                .idRegistroAfectado(t.getIdTransferencia())
                                                .detalle(detalle)
                                                .build());
        }
        ...
        private void validarStockDisponibleEnOrigen(
                        Long idAlmacenOrigen,
                        java.util.List<TransferenciaCrearDTO.Item> items) {

                for (var it : items) {
                        var exOpt = existenciaRepo.lockByAlmacenAndIdLote(idAlmacenOrigen, it.getIdLote());
                        if (exOpt.isEmpty()) {
                                throw new IllegalArgumentException(
                                                "No existe existencia del lote " + it.getIdLote()
                                                                + " en el almacén de origen");
                        }
                        var ex = exOpt.get();
                        if (ex.getCantidadDisponible().compareTo(it.getCantidad()) < 0) {
                                throw new IllegalArgumentException(
                                                "Stock insuficiente para lote " + it.getIdLote() +
                                                                " (disp: " + ex.getCantidadDisponible() + ", req: "
                                                                + it.getCantidad() + ")");
                        }
                }
        }
        ...
        public TransferenciaRespuestaDTO transferirYCompletar(TransferenciaCrearDTO dto) {
                validarCrear(dto);
                validarStockDisponibleEnOrigen(dto.getIdAlmacenOrigen(), dto.getItems());
                ...
                transferenciaRepo.save(t);
                // ✅ AUDITORÍA
                audit("crear_completa", t,
                                "Transferencia completa " +
                                                "origen=" + t.getIdAlmacenOrigen() + ", destino="
                                                + t.getIdAlmacenDestino() +
        ...
        public TransferenciaRespuestaDTO confirmarSalida(Long idTransferencia) {
                var t = transferenciaRepo.findById(idTransferencia)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Transferencia no encontrada: " + idTransferencia));
                ...
                dto.setItems(detalles.stream().map(d -> TransferenciaCrearDTO.Item.builder()
                                .idLote(d.getIdLote())
                ...
                transferenciaRepo.save(t);
                // ✅ AUDITORÍA
                audit("confirmar_salida", t,
                                "Salida confirmada: origen=" + t.getIdAlmacenOrigen() +
                                                ", destino=" + t.getIdAlmacenDestino() +
                                                ", items=" + dto.getItems().size());
        ...
        public TransferenciaRespuestaDTO confirmarIngreso(Long idTransferencia) {
                ...
                transferenciaRepo.save(t);
                // ✅ AUDITORÍA
                audit("confirmar_ingreso", t,
                                "Ingreso confirmado: origen=" + t.getIdAlmacenOrigen() +
                                                ", destino=" + t.getIdAlmacenDestino() +
                                                ", items=" + dto.getItems().size());
        ...
        public TransferenciaRespuestaDTO anular(Long idTransferencia, String motivo) {
                ...
                if (items.isEmpty()) {
                        ...
                        // ✅ AUDITORÍA
                        audit("anular", tx, "Anulación sin items. motivo=" + (motivo == null ? "-" : motivo));
                ...
                for (var it : items) {
                        ...
                        switch (estado) {
                                ...
                                case en_transito -> {
                                        ...
                                        var exOrig = existenciaRepo
                                                        .lockByAlmacenAndIdLote(tx.getIdAlmacenOrigen(), idLote)
                                                        .orElseGet(() -> ExistenciaPorLote.builder()
                                        ...
                                        movRepo.save(MovimientoInventario.builder()
                                                        ...
                                                        .observaciones("Anulación transferencia " + idTransferencia
                                                                        + (motivo != null ? " — " + motivo : ""))
                                                        .build());
                                ...
                                case completada -> {
                                        ...
                                        var exDest = existenciaRepo
                                                        .lockByAlmacenAndIdLote(tx.getIdAlmacenDestino(), idLote)
                                                        .orElseThrow(() -> new IllegalStateException(
                                                                        "No existe stock en destino para el lote "
                                                                                        + idLote
                                                                                        + " de la transferencia "
                                                                                        + idTransferencia));
                                        ...
                                        if (exDest.getCantidadDisponible().compareTo(cant) < 0) {
                                                throw new IllegalStateException(
                                                                "Destino no tiene disponible suficiente para revertir el lote "
                                                                                + idLote + ". Disponible="
                                                                                + exDest.getCantidadDisponible()
                                                                                + ", a revertir=" + cant);
                                        }
                                ...
                                default -> throw new IllegalStateException(
                                                "Estado no soportado para anulación: " + estado);
                        }
                }
                ...
                // ✅ AUDITORÍA
                audit("anular", tx,
                                "Anulación desde estado=" + estado.name() +
