...
export default function MonedasPage() {
    ...
    async function cargar() {
        ...
        try {
            ...
            else setErr(e?.message || "Error cargando monedas");
        } finally {
            setLoading(false);
        }
    }
    ...
    async function onSubmit(e: React.FormEvent) {
        ...
        if (creando && !can("monedas:crear")) {
            show403();
            return;
        }
        if (!creando && !can("monedas:actualizar")) {
            show403();
            return;
        }
        ...
        try {
            if (editando) {
                await monedaService.update(editando.idMoneda, payload);
            } else {
                                ...
                                {(data?.content ?? []).map((m, i) => (
                                    <tr key={m.idMoneda ?? m.codigo ?? i} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 text-right space-x-3 whitespace-nowrap">
                                            {can("monedas:actualizar") && (
                                                <button className="font-medium text-blue-600 hover:underline" onClick={() => startEditar(m)}>
                                                    Editar
                                                </button>
                                            )}
                                            {can("monedas:actualizar") && (
                                                <button
                                                    className={
                                                        m.estadoActivo
                                                            ...
                                                            : "font-medium text-emerald-600 hover:underline"
                                                    }
                        ...
                        <form className="space-y-4" onSubmit={onSubmit}>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setForm((f) => ({ ...f, esLocal: e.target.value === "si", tasa: e.target.value === "si" ? "" : f.tasa }))
                                    }
                                >
                            ...
                            <div className="flex gap-2">
                                ...
                                {editando && (
                                    <button
                                        type="button"
                                        onClick={startCrear}
                                        className="h-10 px-4 rounded-md border border-neutral-300 hover:bg-neutral-50"
                                    >
                                        Cancelar
                                    </button>
                                )}
