...
// helper: mapear idMoneda -> código (USD/BS/…)
async function fetchMapaMonedas(): Promise<Record<number, string>> {
  try {
    const r = await http.get<any[]>("/v1/monedas"); // ajusta si tu back cambia
    const arr = r ?? [];
  ...
  async function fetchData() {
    try {
      ...
      const res = await comprasService.listar({
        estado,
        // el backend filtra por proveedorId; aquí el filtro por nombre es en cliente
        desde,
        hasta,
        page,
        size,
  ...
  return (
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            >
              ...
              {["pendiente", "aprobada", "enviada", "parcial", "recibida", "anulada"].map((s) => (
        ...
        <div className="md:hidden space-y-3">
          {(filas ?? []).map((r: any) => (
              <div key={r.idCompra} className="border rounded-lg p-3 bg-white">
        ...
        <div className="hidden md:block border rounded overflow-hidden bg-white">
                  ...
                  <td className="p-2">
                    <div className="flex items-center gap-2">
                      <Link
                          ...
                          title="Abrir"
                          aria-label="Abrir orden de compra"
                      >
                        <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none">
                          <path d="M14 3h7v7M21 3 13 11M5 12v8a1 1 0 0 0 1 1h8" stroke="currentColor" strokeWidth="2" />
                        </svg>
