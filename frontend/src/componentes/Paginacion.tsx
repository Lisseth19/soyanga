  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      ...
      <div className="flex items-center gap-2">
        <button className="px-3 py-1.5 rounded-lg border disabled:opacity-50"
          onClick={()=>setPage(Math.max(0, page-1))} disabled={loading || isFirst}>← Anterior</button>
        <button className="px-3 py-1.5 rounded-lg border disabled:opacity-50"
          onClick={()=>setPage(Math.min((totalPages||1)-1, page+1))} disabled={loading || isLast}>Siguiente →</button>
        <select className="ml-2 border rounded-lg px-2 py-1.5" value={size}
